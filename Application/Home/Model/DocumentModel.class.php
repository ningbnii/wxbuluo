 ...
 * 文档基础模型
 */
class DocumentModel extends RelationModel{
	...
	protected $_link = array(
        ...
        'document_article' => array(
        	...
        	'mapping_name' => 'document_article',
        	'foreign_key'  => 'id',
        	'mapping_fields'=>array('content,img_url'),
        	'as_fields'	   => 'content,img_url',
        	),		
		);
	 ...
	 * @param  integer  $limit    分页显示，每页的列表数量
	 * @param  integer  $page     显示第几页，默认显示第一页
	 * @param  boolean  $count    是否返回总数
	 * @param  string   $field    字段 true-所有字段
	 * @return array              文档列表
	 */
	public function lists($category, $limit, $field = true, $order = '`id` DESC', $status = 1){
		$listcount = $this->listCount($category, $status);
		$page = new \Think\Page($listcount, $limit);
		$page->setConfig('theme', '%FIRST% %UP_PAGE%  %LINK_PAGE% %DOWN_PAGE% %END%');
        $show = $page->show();
		$map = $this->listMap($category, $status);
		$lists = $this->relation(true)->field($field)->where($map)->limit($page->firstRow.','.$page->listRows)->order($order)->select();
		$data[1] = $lists;
		$data[2] = $show;
		return $data;
	}
	 ...
	 * 获取最新文档
	 * @param integer   $category  分类ID
	 * @param integer   $limit     查询几条数据
	 * @param string    $field     字段   true-表示所有字段
	 * @param string    $order     排序规则
	 * @param integer   $status    状态
	 */
	public function recent($category, $limit, $field = true, $order = '`id` DESC', $status = 1){
		//where条件,根据
		$map = $this->listMap($category, $status);
		$lists = $this->relation(true)->field($field)->where($map)->limit($limit)->order($order)->select();
		return $lists;
	}
